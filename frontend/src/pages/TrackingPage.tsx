...
// Auto-pan map to latest position
...
interface RouteOptimizerProps { token?: string | null }
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Stops <span className="text-gray-400 font-normal">(lat,lon,address — one per line)</span>
          </label>
          <textarea
            className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent resize-none"
            rows={4}
            value={stopsText}
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-brand-50 rounded-xl p-3 text-center">
              <div className="text-lg font-bold text-brand-700">{result.totalDistanceKm} km</div>
              <div className="text-xs text-brand-600">Total Distance</div>
            </div>
            <div className="bg-indigo-50 rounded-xl p-3 text-center">
              <div className="text-lg font-bold text-indigo-700">{result.estimatedTimeFormatted}</div>
              <div className="text-xs text-indigo-600">Estimated ETA</div>
            </div>
          </div>
          <div>
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Optimized Order</div>
            <div className="space-y-1.5">
              {result.orderedStops.map((stop: Stop, i: number) => (
                <div key={stop.id} className="flex items-center gap-2 text-sm">
                  <div className="w-5 h-5 bg-brand-500 text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {i + 1}
                  </div>
                  <span className="text-gray-700 truncate">{stop.address}</span>
                  {i < result.segments.length && (
                    <span className="text-xs text-gray-400 ml-auto flex-shrink-0">{result.segments[i].distanceKm}km</span>
                  )}
                </div>
              ))}
            </div>
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Real-time driver location and route history</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            {/* Search */}
            <div className="card">
              <h3 className="font-semibold text-gray-900 mb-3 text-sm">Track an Order</h3>
              <div className="flex gap-2">
                ...
                />
                <Button onClick={handleTrack} size="md">
                  <Search size={15} />
                </Button>
              </div>
            </div>
            ...
            {activeOrderId && (
              <div className="card">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-gray-900 text-sm">Tracking Status</h3>
                  <div className={`flex items-center gap-1.5 text-xs font-medium ${isConnected ? 'text-emerald-600' : 'text-gray-400'}`}>
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-3">
                    <div className="bg-emerald-50 rounded-xl p-3 flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-xs text-emerald-600 font-medium">Current Position</div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-gray-50 rounded-xl p-2.5">
                        ...
                        <div className="font-semibold text-gray-700 truncate">{latest.driverId}</div>
                      </div>
                      <div className="bg-gray-50 rounded-xl p-2.5">
                        <div className="text-gray-400">Points</div>
                        <div className="font-semibold text-gray-700">{history.length}</div>
                      </div>
                    </div>
            ...
            {history.length > 0 && (
              <div className="card">
                <h3 className="font-semibold text-gray-900 text-sm mb-3">
                  Route History <span className="text-gray-400 font-normal">({history.length} points)</span>
                </h3>
                <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                  {[...history].reverse().slice(0, 20).map((p) => (
                    <div key={p._id} className="flex items-center gap-2 text-xs py-1.5 border-b border-gray-50 last:border-0">
                      <div className="w-4 h-4 bg-brand-100 rounded-full flex items-center justify-center flex-shrink-0">
                        <div className="w-1.5 h-1.5 bg-brand-500 rounded-full" />
                      </div>
                      <span className="text-gray-500">{p.latitude.toFixed(4)}, {p.longitude.toFixed(4)}</span>
                      <span className="text-gray-300 ml-auto">{new Date(p.createdAt).toLocaleTimeString()}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="lg:col-span-2">
            ...
            {!activeOrderId && (
              <div className="mt-4 card bg-navy-50 border-navy-100">
                <div className="flex items-start gap-3">
                  <AlertCircle size={18} className="text-navy-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <div className="text-sm font-semibold text-navy-800">Enter an Order ID to start tracking</div>
                    <div className="text-xs text-navy-600 mt-1">
                      You can find order IDs on the Orders page. The map will update in real-time as the driver moves.
                    </div>
                  </div>
                </div>
              </div>
