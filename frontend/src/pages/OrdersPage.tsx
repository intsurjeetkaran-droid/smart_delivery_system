...
// ── Create Order Modal ────────────────────────────────────────────────────────
interface CreateOrderModalProps { isOpen: boolean; onClose: () => void; onCreate: (data: any) => Promise<void> }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input label="Customer Name" placeholder="Rahul Sharma" value={form.customerName}
            onChange={e => setForm(p => ({ ...p, customerName: e.target.value }))} icon={<User size={15} />} required />
          <Input label="Phone Number" placeholder="9876543210" value={form.customerPhone}
            onChange={e => setForm(p => ({ ...p, customerPhone: e.target.value }))} icon={<Phone size={15} />} required />
        </div>
        <Input label="Pickup Address" placeholder="12 MG Road, Bangalore" value={form.pickupAddress}
          onChange={e => setForm(p => ({ ...p, pickupAddress: e.target.value }))} icon={<MapPin size={15} />} required />
        <Input label="Delivery Address" placeholder="45 Anna Salai, Chennai" value={form.deliveryAddress}
          onChange={e => setForm(p => ({ ...p, deliveryAddress: e.target.value }))} icon={<MapPin size={15} />} required />
...
// ── Assign Driver Modal ───────────────────────────────────────────────────────
interface AssignDriverModalProps { isOpen: boolean; onClose: () => void; order: Order | null; onAssign: (orderId: string, driverId: string) => Promise<void> }
...
const AssignDriverModal: React.FC<AssignDriverModalProps> = ({ isOpen, onClose, order, onAssign }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!order) return; setError(''); setLoading(true)
    try { await onAssign(order._id, driverId); setDriverId(''); onClose() }
    catch (err: any) { setError(err.response?.data?.message || 'Failed to assign driver') }
    finally { setLoading(false) }
  }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Assign Driver" size="sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Assigning driver to order for <strong className="text-gray-900 dark:text-gray-100">{order?.customerName}</strong>
        </p>
        <Input label="Driver ID" placeholder="driver_ravi_001" value={driverId}
          onChange={e => setDriverId(e.target.value)} icon={<Truck size={15} />} required />
        {error && <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm rounded-xl px-4 py-3">{error}</div>}
        <div className="flex gap-3 pt-2">
          <Button type="button" variant="outline" className="flex-1" onClick={onClose}>Cancel</Button>
...
// ── Order Card ────────────────────────────────────────────────────────────────
interface OrderCardProps { order: Order; role: string; onStatusUpdate: (id: string, status: string) => void; onAssignDriver: (order: Order) => void }
    ...
    <div className="card hover:shadow-card-hover transition-all duration-200 animate-fade-in">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-navy-50 dark:bg-navy-900/40 rounded-xl flex items-center justify-center flex-shrink-0">
            <Package size={18} className="text-navy-600 dark:text-navy-300" />
          </div>
          <div>
            <div className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{order.customerName}</div>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-start gap-2 text-sm">
          <div className="w-5 h-5 bg-brand-100 dark:bg-brand-900/40 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
            <div className="w-1.5 h-1.5 bg-brand-500 rounded-full" />
          </div>
          <div>
            <div className="text-xs text-gray-400 dark:text-gray-500 font-medium">PICKUP</div>
            <div className="text-gray-700 dark:text-gray-300">{order.pickupAddress}</div>
          </div>
        </div>
        <div className="ml-2.5 w-px h-4 bg-gray-200 dark:bg-gray-700" />
        <div className="flex items-start gap-2 text-sm">
          ...
          <div>
            <div className="text-xs text-gray-400 dark:text-gray-500 font-medium">DELIVERY</div>
            <div className="text-gray-700 dark:text-gray-300">{order.deliveryAddress}</div>
          </div>
        </div>
      </div>

      {order.assignedDriver && (
        <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 rounded-xl px-3 py-2 mb-4">
          <Truck size={14} className="text-blue-600 dark:text-blue-400" />
          <span className="text-xs text-blue-700 dark:text-blue-400 font-medium">Driver: {order.assignedDriver}</span>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-950">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-52 bg-white dark:bg-gray-900 rounded-2xl animate-pulse border border-gray-100 dark:border-gray-800" />
            ))}
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(order => (
              <OrderCard key={order._id} order={order} role={user?.role || 'customer'}
      ...
      <AssignDriverModal isOpen={assignOpen} onClose={() => setAssignOpen(false)}
        order={selectedOrder} onAssign={async (id, did) => { await assignDriver(id, did) }} />
    </div>
  )
}
