    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Good day, <span className="text-brand-600">{user?.name?.split(' ')[0]}</span> 👋
            </h1>
          ...
          <Link
            to="/orders"
            className="inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-sm"
          >
            <Plus size={16} />
            New Order
          </Link>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard label="Total Orders"  value={stats.total}     icon={<Package size={22} />}     color="text-navy-600"   bg="bg-navy-100" />
          <StatCard label="Pending"       value={stats.pending}   icon={<Clock size={22} />}        color="text-amber-600"  bg="bg-amber-50" />
          <StatCard label="In Transit"    value={stats.inTransit} icon={<Truck size={22} />}        color="text-indigo-600" bg="bg-indigo-50" />
          <StatCard label="Delivered"     value={stats.delivered} icon={<CheckCircle size={22} />}  color="text-emerald-600" bg="bg-emerald-50" />
        </div>
        ...
        <div className="card">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <Link to="/orders" className="text-brand-600 text-sm font-medium mt-2 inline-block hover:underline">
            ...
            <div className="space-y-2">
              {recent.map((order: Order) => (
                <div key={order._id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <Link
                      ...
                      className="text-xs text-brand-600 hover:underline mt-0.5 block opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      Track →
                    </Link>
                  </div>
        ...
        {user?.role === 'admin' && (
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Link to="/orders" className="card hover:shadow-card-hover transition-shadow group flex items-center gap-4">
              ...
              <div>
                <div className="font-semibold text-gray-900 text-sm">Manage Orders</div>
                <div className="text-xs text-gray-400">Assign drivers, update statuses</div>
              </div>
              <ArrowRight size={16} className="text-gray-300 ml-auto group-hover:text-brand-500 transition-colors" />
            </Link>
            <Link to="/tracking" className="card hover:shadow-card-hover transition-shadow group flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-400">Monitor driver locations</div>
              </div>
              <ArrowRight size={16} className="text-gray-300 ml-auto group-hover:text-indigo-500 transition-colors" />
            </Link>
          </div>
        )}
      </main>
